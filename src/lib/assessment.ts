...
import { scorePt7DebugAnswer, scorePt7TeddyAnswer } from "./pt7";
import type {
  AssessmentItem,
  AssessmentResult,
  AssessmentSection,
...
const scoreDebugBlockTask = (item: AssessmentItem, selectedAnswer: SelectedAnswer) => {
  return scorePt7DebugAnswer({
    itemId: item.id,
    wrongBlockIds: item.blockTask?.wrongBlockIds ?? [],
    repairChecks: item.blockTask?.debugRepairChecks ?? [],
    answer: selectedAnswer,
  });
};

const scoreBlockTask = (item: AssessmentItem, selectedAnswer: SelectedAnswer) => {
  if (item.blockTask?.itemVersion === "pt7-teddy-build-v1" && item.blockTask.teddyWorld) {
    return scorePt7TeddyAnswer({
      answer: selectedAnswer,
      world: item.blockTask.teddyWorld,
    });
  }
